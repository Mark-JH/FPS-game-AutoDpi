...
def build_overlay(sample_size: int) -> tuple[tk.Tk, tk.Canvas, tk.Label]:
    root = tk.Tk()
    ...
    canvas.create_rectangle(left, top, right, bottom, outline="cyan", width=2)
    ...
    return root, canvas, status_label


def update_overlay(status_label: tk.Label, state: State, target_dpi: int, default_dpi: int) -> None:
    status_text = (
        f"Enabled: {'ON' if state.enabled else 'OFF'}\n"
        f"Current DPI: {state.current_dpi}\n"
        f"Target DPI: {target_dpi}\n"
        f"Default DPI: {default_dpi}"
    )
...
def main() -> None:
    ...
    overlay_root, _, status_label = build_overlay(args.sample_size)
    ...
    def loop() -> None:
        with mss.mss() as sct:
            ...
            while True:
                ...
                update_overlay(status_label, state, args.target_dpi, args.default_dpi)
